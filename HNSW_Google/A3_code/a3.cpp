...
int main(int argc, char *argv[])
{
	...
	if (rank == 0)
	{
		...
		for (int i = 0; i < summa; i++)
		{
			if (i < buffer_arr[2])
			{
				// level.push_back(buffer_arr[i]);
				temp_arr[i] = level[i];
			}
			else if (i < buffer_arr[2] + buffer_arr[3])
			{
				// index.push_back(buffer_arr[i]);
				temp_arr[i] = index[i - buffer_arr[2]];
			}
			else if (i < buffer_arr[2] + buffer_arr[3] + buffer_arr[4])
			{
				//	indptr.push_back(buffer_arr[i]);
				temp_arr[i] = indptr[i - buffer_arr[2] - buffer_arr[3]];
			}
			else
			{
				//	index_offset.push_back(buffer_arr[i])
	...
	#pragma omp parallel for
		for (int it = startthread[rank]; it < endthread[rank] + 1; it++)
		{
			...
			// int indpt[indptr.size()];
			// for (int uh = 0; uh < indptr.size(); uh++)
			// {
			// 	indpt[uh] = indptr[uh];
			// }

			// int ind[index.size()];
			// for (int uh = 0; uh < index.size(); uh++)
			// {
			// 	ind[uh] = index[uh];
			// }

			// int offset[level_offset.size()];
			// for (int uh = 0; uh < level_offset.size(); uh++)
			// {
			// 	offset[uh] = level_offset[uh];
			// }

			//cout<<"Next start query\n";
